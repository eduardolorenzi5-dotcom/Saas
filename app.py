...
def enviar_wpp_boas_vindas(whatsapp, nome):
    ...
    mensagem = (
        ...
        f"_\"Entrou um freela de 500\"_\n\n"
        f"📊 *Ver saldo e resumo:*\n"
        f"_\"Saldo\"_ ou _\"Resumo do mês\"_\n\n"
        f"⏰ *Lembretes mensais:*\n"
        f"_\"Me lembre todo dia 10 de pagar a conta de energia\"_\n"
        f"_\"Todo dia 5 me lembre do boleto do cartão\"_\n\n"
        ...
        f"🎤 *Áudio:* pode falar o gasto em vez de digitar\n"
        f"📸 *Comprovante:* manda foto que eu registro automático\n"
        f"━━━━━━━━━━━━━━━━\n\n"
        f"Qualquer dúvida é só me perguntar aqui. Vamos começar? 😊"
    )
    payload = _json.dumps({
        "number": numero,
        "text": mensagem,
...
def admin_painel():
    ...
    clientes = conn.execute("""
        ...
        GROUP BY c.id, c.nome, c.email, c.whatsapp, c.status, c.criado_em,
                 c.renda_mensal, c.google_refresh_token, c.mp_subscription_id, c.trial_expiry, p.nome, p.preco
